...
(async () => {
	...
	await Contest.updateOne({id: 'mayfes2020-marathon'}, {
		...
		start: new Date('2020-09-20T16:03:00+0900'),
		...
		description: {
			...
			* $1 \\leq C_i \\leq 1000$
			...
			\`\`\`
			### 出力
			全体の操作回数を $M$、$j$ 回目の動作で操作する電波塔を電波塔 $w_j$、操作する時間を $t_j$ として、以下のフォーマットに従って標準出力に出力せよ。
			\`\`\`
			M
			w_1 t_1
			...
			w_M t_M
			\`\`\`
			なお、 $M$, $w_j$, $t_j$ は以下の条件を満たしている必要がある。条件を満たしていない提出は受理されない。
			* 出力はすべて整数
			* $1 \\leq M \\leq 200000$
			...
			* $M$ 回目の操作の終了後、起動されていない電波塔は存在しない
			...
			受理された提出は、$T=\\Sigma_{j=1}^M\\ t_j$ がスコアとして付与される。**スコアが小さい方が最終得点が高い**ことに注意。最終得点の詳細は「スコア評価」を参照すること。
			...
			* これで合計時間 $T = 168$ で全ての電波塔が起動するため、この提出のスコアは $168$ となる。
			...
			## テストケース生成方法
			テストケースは全部で**17個**ある。そのうち 16 個のテストケースは**提出毎にランダムで生成**される。
			
			すべてのテストケースにおいて、電波塔の座標は $[-2000, 2000] \\times [-2000,2000]$ の範囲から無作為に選ばれた整数のペア $(X,Y)$ である。
			電波塔の個数 $N$ およびコスト $C_i$ の条件は各テストケースによって異なり、以下のような内訳となっている。
			
			### small-static (1個)
			- $N=20$
			- 提出毎に**内容が変化しない唯一のテストケース**である。
			- 入力は以下の通り与えられる。
			\`\`\`
			20
			-1695 514 334
			...
			## スコア評価
			この問題の最終点数 $S$ は、各テストケースにおけるスコアを $T_i\ (1 \\leq i \\leq 17)$ とし、また入力の制約から導出される最悪のスコア $WORST=3.2\\times10^{10}$ を用いて以下の式によって計算される。

			$S = \\displaystyle\\sum_{i=1}^{17}round\\left(10000\\times\\left(1-\\left(\\frac{T_i}{WORST}\\right)^{1/4}\\right)\\right)$

			ただし提出が受理されなかった場合や $T_i > WORST$ である場合、$T_i = WORST$であるものとして計算される。上の式から分かる通り、この場合そのテストケースによる最終得点 $S$ への寄与は $0$ となる。
			`,
			en: '',
		},
	}, {upsert: true}, (err) => {
