	...
	await Contest.updateOne({id: 'mayfes2020-marathon'}, {
		...
		description: {
			...
			* $M$ 回目の操作の終了後、起動されていない電波塔は存在しない
			...
			受理された提出は、$T=\\Sigma_{j=1}^M t_j$ がスコアとして付与されます。
			...
			* これで合計時間 $T = 168$ で全ての電波塔が起動するため、この提出はスコア $168$ を得る。
			...
			## スコア評価
			この問題の最終点数は**相対評価**によって決まる。各テストケースにおいて、自分の提出したプログラムのうち、受理された出力のスコアの最小値を $T_{s}$ (受理された出力がない場合は $T_s = \\infty$ )、同様に相手のスコアの最小値を $T_{o}$ とすると、そのテストケースであなたは以下の点数を得ることができる。

			- $T_s < T_o$ の場合**1点**
			- $T_s > T_o$ の場合**0点**
			- $T_s = T_o$ の場合**0.5点** (ただし、双方受理された出力のプログラムを書けなかった場合は**0点**)

			全15テストケースでの点数を合計し、点数の高かった者が TSG Live! 5 ライブ競技プログラミング部門の勝者となる。点数が引き分けの場合、受理された出力が提出された時刻の和が小さいほうが勝者となる。
			`,
			en: '',
		},
	}, {upsert: true}, (err) => {
