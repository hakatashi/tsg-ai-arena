...
const evaluation = (infixFormula) => {
    ...
    let topRawNumber = 0;
    const operation = () => {
        if (operandStack.length < 2) throw "InvalidFormula";
        ...
        if (operator === "+") {
            ...
            if (bigRat(0).compare(lhs) == 0) throw "InvalidFormula";
            topRawNumber = 0;
            operandStack.push(rhs.divide(lhs));
        } else {
            if (topRawNumber < 2) throw "InvalidFormula";
            --topRawNumber;
    ...
    for (const token of infixFormula) {
        if (token === "+" || token === "-") {
            ...
            if (operatorStack.length === 0) throw "InvalidFormula";
            operatorStack.pop();
        } else {
    ...
    while (operatorStack.length > 0) {
        if (!level2Operator.includes(operatorStack[operatorStack.length - 1])) throw "InvalidFormula";
        operation();
    }
    if (operandStack.length !== 1) throw "InvalidFormula";
