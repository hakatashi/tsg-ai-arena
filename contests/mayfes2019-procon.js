...
const initMaps(height, width) => {
  const field = Array(width * height).map(Math.floor(Math.random() * 201)-100);
  return field;
};
...
mocule.exports.isInside = isInside;
...
module.exports.battler = async (
  ...
  const {stdout} = wait execute(serialize({params, state:initialState}));
