...
const {createLineDrainer, transaction} = require('../lib/utils.js');
...
const dequeue = async () => {
	...
	if (battle.match !== null) {
		await transaction(async () => {
			...
			const results = matchConfigs.map((matchConfig) => {
				const battle = battles.find(({players, config}) => (
					matchConfig.config === config &&
					matchConfig.players.every((playerIndex, index) => match.players[playerIndex]._id.equals(players[index]))
				));
				if (!battle) {
...
module.exports.enqueue = async ({players, contest, user, config, match = null}) => {
